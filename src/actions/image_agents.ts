...
export const imagePreprocessAgent = async (namedInputs: {
  ...
  if (beat.image) {
    ...
    if (isAnimatedHtml) {
      ...
      const hasDuration = returnValue.beatDuration !== undefined;
      if (hasDuration) {
        // Movie workflow: plugin generates .mp4, extractImageFromMovie extracts thumbnail
        return {
          ...returnValue,
          imagePath,
          movieFile: animatedVideoPath,
          imageFromMovie: true,
          useLastFrame: true,
      ...
      // PDF/images workflow (no duration): plugin generates final-frame PNG only
      return {
        ...returnValue,
        imagePath,
        referenceImageForMovie: pluginPath,
...
export const imagePluginAgent = async (namedInputs: {
  ...
  // For animated html_tailwind with known duration, use the .mp4 path so the plugin writes video there.
  // Without duration (PDF/images workflow), use the .png path for final-frame rendering.
  const isAnimatedHtml = MulmoBeatMethods.isAnimatedHtmlTailwind(beat);
  const studioBeatForPath = context.studio.beats[index];
  const hasDuration = (beat.duration ?? studioBeatForPath?.duration) !== undefined;
  const effectiveImagePath = isAnimatedHtml && hasDuration ? getBeatAnimatedVideoPath(context, index) : imagePath;
